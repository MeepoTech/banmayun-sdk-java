...
public class JsonArrayReader<T, L> extends JsonReader<L> {
  ...
  public JsonArrayReader(JsonReader<? extends T> elementReader,
      Collector<T, ? extends L> collector) {
    this.elementReader = elementReader;
    this.collector = collector;
  }

  public static <T> JsonArrayReader<T, ArrayList<T>> mk(
      JsonReader<? extends T> elementReader) {
    return new JsonArrayReader<T, ArrayList<T>>(elementReader,
        new Collector.ArrayListCollector<T>());
  }

  public static <T, L> JsonArrayReader<T, L> mk(
      JsonReader<? extends T> elementReader, Collector<T, ? extends L> collector) {
  ...
  public static <T, L> L read(JsonReader<? extends T> elementReader,
      Collector<T, ? extends L> collector, JsonParser parser)
