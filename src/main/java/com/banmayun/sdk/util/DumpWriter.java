...
public abstract class DumpWriter {
    ...
    public Multiline(StringBuilder buf, int indentAmount, int currentIndent,
      ...
      if (indentAmount < 0)
        throw new IllegalArgumentException(
            "'indentAmount' must be non-negative");
      if (currentIndent < 0)
        throw new IllegalArgumentException(
            "'currentIndent' must be non-negative");
      this.buf = buf;
  ...
  public static String toStringDate(Date date) {
    if (date == null) {
      ...
      buf.append('"');
      buf.append(year).append("/").append(month).append("/").append(day)
          .append(" ");
      buf.append(hour).append(":").append(minute).append(":").append(second)
          .append(" UTC");
      buf.append('"');
      return buf.toString();
    }
